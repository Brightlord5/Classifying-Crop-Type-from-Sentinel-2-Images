            ...
            try:
                ...
                with overview_tab:
                    ...
                    with col1:
                        ...
                        sizes = area_df['Area (hectares)']
                        percentages = (sizes / sizes.sum()) * 100
                        
                        def shorten_label(crop_type, percentage):
                            if percentage < 3:
                                return ''
                            if 'intercropping' in crop_type.lower():
                                return crop_type.split('(')[0].strip()
                            return crop_type

                        labels = [shorten_label(crop, pct) for crop, pct in zip(area_df['Crop Type'], percentages)]
                        ...
                        plt.title('Distribution of Crop Types by Area', pad=20, size=14)
                ...
                with area_tab:
                    ...
                    bars = plt.bar(area_df['Crop Type'], area_df['Area (hectares)'])
                    plt.xticks(rotation=45, ha='right')
                    plt.ylabel('Area (hectares)')
                    plt.title('Area Coverage by Crop Type')
                            ...
                            with Timer(f"Processing tile {tile_str}") as timer:
                                ...
                                if rgb_image is not None:
                                    with viz_container:
                                        ...
                                        st.pyplot(fig)
                                        save_path = os.path.join('visualizations', f'image_{date}_{i}.png')
